<template>
  <v-container id="page-articles"
    ...
    <div :style="{
      ...
      <!-- <ItemCardList :keys="'page-articles-recent'" :type="'articles'" :item_data="10" v-if="(!$store.getters.App.IsLogin && $route.hash == '') ||
        (!$store.getters.App.IsLogin && $route.hash == '#recent') ||
        ($store.getters.App.IsLogin && $route.hash == '') ||
        ($store.getters.App.IsLogin && $route.hash == '#recent')
        " />
      <ItemCardList :keys="'page-articles-popular'" :type="'articles'" :item_data="20"
        v-if="(!$store.getters.App.IsLogin && $route.hash == '#popular') || ($store.getters.App.IsLogin && $route.hash == '#popular')" />
      <ItemCardList :keys="'page-articles-following'" :type="'articles'" :item_data="30"
          ...
          >
            <v-list class="layout-colourless" style="padding: 0;">
              <ListItem type="articles"
                v-for="i in 15"
                :url="`${$G_UrlHeaderLang()}/articles/1`"
                />
            </v-list>
          </v-card>
...
export default {
  ...
  methods: {
    SwitchWebTitle(to) {
      if (to.path == `${this.$G_UrlHeaderLang()}/articles`) {
        this.$G_SetWebSubTitle('Message.Pages.Articles.WebSubTitle', true)
        if (to.hash == '#recent' || to.hash == '') {
          this.tab = 'recent'
          this.$G_SetWebSubTitle('Message.Pages.Articles.LastestArticles')
        } else if (to.hash == '#popular') {
          this.tab = 'popular'
          this.$G_SetWebSubTitle('Message.Pages.Articles.RecentlyPopularArticles')
        } else if (this.$store.getters.App.IsLogin && to.hash == '#following') {
          this.tab = 'following'
          this.$G_SetWebSubTitle('Message.Pages.Articles.ArticlesIAmFollowing')
        }
      }
    }
  },
