<template>
  <v-expand-transition>
    <v-card v-show="$store.getters.App.Editor.State == 'open' ||
      ...
      <div class="body">
        <v-text-field :placeholder="$t('Message.Components.Editor.PleaseEnterATitle')" v-show="withTitle" />
        <div class="editor-topics" v-show="withTopics" @click="$store.getters.App.Dialog.TopicSelector = !$store.getters.App.Dialog.TopicSelector">
          <!-- <TopicChip /> -->
        ...
        <div class="editor-toolbar">
          <v-menu offset-y open-on-hover :rounded="$store.getters.App.Style.rounded">
        ...
        <div class="editor-toolbar">
          <v-btn color="accent" @click="OnEditorSubmit()" :disabled="editor.getText() == ''">
            {{ action_text ? action_text : $t('Message.Components.Editor.Release') }}
          </v-btn>
...
//import TopicChip from './components/topic-chip/index.vue'
//import TopicSelectorDialog from './components/topic-selector-dialog/index.vue'
import Link from './components/to-link/index.vue'
//import CustomLinkExtension from './components/to-link/CustomLinkExtension';
export default {
  name: 'md-editor',
  components: {
    EditorContent,
  ...
  props:{
    ...
    mode:{
      type: String,
      default: 'auto',//auto,articles,questions,answer
    }
  },
  data: () => ({
    withTitle: true,
    ...
    maximize: false,
    editor: null,
    EditorTitle: 'Message.Components.Editor.NewArticles',
  }),
  created() {
        ...
        <pre><code class="language-javascript">for (var i=1; i <= 20; i++)
{
  ...
  methods: {
    ...
    OnEditorSubmit() {
      ...
      this.SaveContentToLocal()
      var html = this.editor.getHTML()//content_rendered 
      var text = this.editor.getText()//content_markdown
      console.log(html)
      console.log(text)
    },
  },
  watch: {
    ...
    //   switch(val){
    //     case 'articles':
    //       this.EditorTitle = 'Message.Components.Editor.NewArticles'
    //       this.withTitle = true
    //       this.withTopics = true
    //       if (localStorage.getItem('articles_cache')) {
    //         this.editor.commands.setContent(JSON.parse(localStorage.getItem('articles_cache')))
    //         this.$store.getters.App.Editor.State = 'minimize'
    //       } else {
    //         //this.editor.commands.setContent('')
    //       }
    //       break;
    //     case 'questions':
    //       this.EditorTitle = 'Message.Components.Editor.NewQuestions'
    //       this.withTitle = true
    //       this.withTopics = true
    //       if (localStorage.getItem('questions_cache')) {
    //         this.editor.commands.setContent(JSON.parse(localStorage.getItem('questions_cache')))
    //         this.$store.getters.App.Editor.State = 'minimize'
    //       } else {
    //         //this.editor.commands.setContent('')
    //       }
    //       break;
    //     case 'answer':
    //       this.EditorTitle = 'Message.Components.Editor.WriteAnAnswer'
    //       this.withTitle = false
    //       this.withTopics = false
    //       if (localStorage.getItem('answer_cache')) {
    //         this.editor.commands.setContent(JSON.parse(localStorage.getItem('answer_cache')))
    //         this.$store.getters.App.Editor.State = 'minimize'
    //       } else {
    //         //this.editor.commands.setContent('')
    //       }
    //       break;
    //     default:
    //       this.EditorTitle = 'Message.Components.Editor.HeaderTitle'
    //       this.SaveContentToLocal()
    //       this.$store.getters.App.Editor.State = 'close'
    //       //this.editor.commands.setContent('')
    //       break;
    ...
    // },
    '$route'(to, from) {
      if (this.editorTitle != ''&&this.mode!='auto') {
        this.EditorTitle = this.editorTitle
        
        return
      }
      if (to.path == `${this.$G_UrlHeaderLang()}/articles`||to.hash.indexOf('articles') != -1) {
        this.EditorTitle = 'Message.Components.Editor.NewArticles'
        this.withTitle = true
        this.withTopics = true
        if (localStorage.getItem('articles_cache')) {
      ...
      if (to.path == `${this.$G_UrlHeaderLang()}/questions`||to.hash.indexOf('questions') != -1) {
        this.EditorTitle = 'Message.Components.Editor.NewQuestions'
        this.withTitle = true
        this.withTopics = true
        if (localStorage.getItem('questions_cache')) {
